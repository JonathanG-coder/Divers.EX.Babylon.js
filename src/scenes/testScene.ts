...
export class TestScene implements CreateSceneClass {
   createScene = async (
      ...
      // Check the equality of the shapes and switch the light when the shapes are the same
      function switchLight(): void {
         leftSwitch.image &&
         rightSwitch.image &&
         leftSwitch.image.source === rightSwitch.image.source
      ...
      leftSwitch.onPointerUpObservable.add(() => {
         // Switch the shape
         if (leftSwitch.image && leftSwitch.image.source === point) {
            leftSwitch.image.source = line;
            switchLight();
         } else if (leftSwitch.image && leftSwitch.image.source === line) {
            leftSwitch.image.source = point;
            switchLight();
         }
      });
      ...
      rightSwitch.onPointerUpObservable.add(() => {
         // Switch the shape
         if (rightSwitch.image && rightSwitch.image.source === point) {
            rightSwitch.image.source = line;
            switchLight();
         } else if (rightSwitch.image && rightSwitch.image.source === line) {
            rightSwitch.image.source = point;
            switchLight();
         }
      });
