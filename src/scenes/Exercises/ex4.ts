...
// Assets
import ABCData from "../../assets/data/ABC.json";

interface LetterPatterns {
   [key: string]: string;
}

const ABC: LetterPatterns = ABCData;
   ...
   createScene = async (
      ...
      for (let row = 0; row < y; row++) {
         for (let col = 0; col < x; col++) {
            ...
            led.position.x = (x - col - 1) * spacing;
            // led.position.y = row * spacing;
            led.position.y = (y - row - 1) * spacing;
      ...
      function updateBillboard(text: string) {
         // Clear all LEDs
         for (let led of leds) {
            (led.material as StandardMaterial).emissiveColor = Color3.Black();
         }
         ...
         for (let i = 0; i < text.length; i++) {
            ...
            const pattern = ABC[char].replace(/\s/g, "");

            if (pattern) {
               for (let j = 0; j < pattern.length; j++) {
                  if (pattern[j] === "1") {
                     (leds[j].material as StandardMaterial).emissiveColor =
                        Color3.Red();
                  }
               }
            }
