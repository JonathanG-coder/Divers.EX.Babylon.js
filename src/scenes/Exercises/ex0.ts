...
export class Ex0 implements CreateSceneClass {
   createScene = async (
      ...
      const fontData = await (
         await fetch("https://assets.babylonjs.com/fonts/Droid Sans_Bold.json")
      ).json();
      ...
      // Create a random number between 1 and 10 and trigger the animation when the user enters the correct number then hits enter
      let animate: boolean = false;
      input.onKeyboardEventProcessedObservable.add(({ key }) => {
         const randomNumber: string = Math.floor(Math.random() * 11).toString();

         if (input.text === randomNumber && key === "Enter") {
            pbr.roughness = 0;
